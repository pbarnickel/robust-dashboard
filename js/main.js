$(document).ready(function () {
    aHistoryData = $.getJSON("/js/data.json", function (oJSON) {
        initTotalBurnHistoryChart(oJSON);
        initBurnHistoryChart(oJSON);
        initBurnHistoryChartCurrentYear(oJSON);
    });
...
//chart 3
function initBurnHistoryChartCurrentYear(oJSON) {
    ...
    for (var i = 0; i < oJSON.length; i++) {
        oItem = {
            date: d3.timeParse("%d.%m.%Y")(oJSON[i].Date),
            value: oJSON[i].Burned
        }

        // push only datasets of current year
...
//chart 2
function initTotalBurnHistoryChart(oJSON) {
    ...
    // Add Y axis
    var y = d3.scaleLinear()
        .domain([6750, d3.max(data, function (d) {
            return +d.value;
        })])
        .range([height, 0]);
    yAxis = svg.append("g")
...
//chart 1
function initBurnHistoryChart(oJSON) {
